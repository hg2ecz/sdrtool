...
fn main() {
    ...
    let mut allread = 0;
    loop { // reduce buffering-delay, empty TCP channel
    ...
    loop {
        if let Ok(size) = tcp.read(&mut buf_in) {
            ...
            for i in (0..buf.len()-1).step_by(2) {
                sndbuf.push((buf[i+1] as i16) << 8 + 0 as i16);
                bufptr = i;
            }
            buf.drain(..bufptr);
        ...
        // command to server
        if let Some(data_in) = cmd.getstring() { // if new command ...
            let mut data = data_in;
            data.push('\n');
            data.push('\0');
            tcp.write(data.as_bytes()).unwrap();
        }
